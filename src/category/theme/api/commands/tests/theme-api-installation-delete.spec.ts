...
import { ThemeApiInstallationDeleteCommand } from "../theme-api-installation-delete";
...
describe("ThemeApiInstallationDeleteCommand", () => {
	...
	it("calls deleteInstallation with -y", async () => {
		themeApiCmdMocks.tryLoadResult = {
			success: true,
			config: {
				publicApiToken: "t",
		...
		await parseWithTail(program, ["theme", "delete", "-y"]);
		expect(themeApiCmdMocks.deleteInstallation).toHaveBeenCalledWith("10");
		expect(themeApiCmdMocks.log).toHaveBeenCalledWith(
			"Theme 10 deleted successfully.",
		);
	});
	...
	it("writes JSON when --json", async () => {
		...
		themeApiCmdMocks.deleteInstallation.mockResolvedValue({ ok: true });
		const program = programWithThemeCommand((c) => {
			new ThemeApiInstallationDeleteCommand().Bind(c);
		});
		await parseWithTail(program, ["theme", "delete", "-y", "--json"]);
	...
	it("accepts deprecated --installation-id and warns on stderr", async () => {
		...
		expect(themeApiCmdMocks.deleteInstallation).toHaveBeenCalledWith("77");
